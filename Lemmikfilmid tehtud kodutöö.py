...
hetkesisestus = "a"
while(hetkesisestus != ""):
    hetkesisestus = input("Sisestage enda lemmikud filmid,kui rohkem ei ole,vajutage enter").capitalize()
    if hetkesisestus != "":
        lemmikfilmid.append(hetkesisestus)
print(lemmikfilmid)
KõigeLemmikumFilm = ""
while(KõigeLemmikumFilm == ""):
    KõigeLemmikumFilm = input("\nMis on sinu kõige lemmikum film kogu aegade jooksul? ").capitalize()
    if KõigeLemmikumFilm in lemmikfilmid:
        print("Ossa, oled oma lemmiku isegi kaks korda pannud!")
    else:
        print("Aga kus on sinu " + str(KõigeLemmikumFilm) + "?")
        muljetavaldav = input("Kas sulle meeldib " + str(KõigeLemmikumFilm) + "? (jah/ei): ")
        if muljetavaldav == "jah":
            lemmikfilmid.append(KõigeLemmikumFilm.strip())
            print("Lisati " + str(KõigeLemmikumFilm) + " ka sinu lemmikfilmide nimekirja!")
        else:
...
print("Sul on kokku " + str(filmide_arv) + " lemmikfilmi.")
if filmide_arv <= 5:
